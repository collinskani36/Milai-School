...
// ─── Fixed: joins assessments to get max_marks (column no longer on assessment_results) ───
const fetchSubjectBreakdownForAssessment = async (
  studentId: string,
  assessmentId: string
) => {
  console.log(`Fetching subject breakdown — assessment: ${assessmentId}, student: ${studentId}`);
  ...
  console.log("Subject breakdown data:", data);
...
// ─── Fixed: dynamic pdf import avoids flushSyncCallbacks error ───────────────
const downloadExamPDF = async (
  ...
  try {
    // Fetch assessment header + ranking in parallel
    const [assessRes, rankingRes] = await Promise.all([
      supabase
        .from("assessments")
        .select("title, term, year, max_marks, category")
        .eq("id", assessmentId)
        .single(),
      // student_rankings holds class_position, total_attained, total_possible
      supabase
        .from("student_rankings")
        .select("class_position, total_attained, total_possible, assessment_date")
        .eq("assessment_id", assessmentId)
        .eq("student_id", studentId)
        .maybeSingle(),
    ]);
    ...
    const ranking    = rankingRes.data; // null if not yet ranked
    ...
    // Prefer ranking totals (authoritative); fall back to summing subject scores
    ...
    const examRecord = {
      ...
      // Class position from rankings — shown on the PDF slip
      classPosition:    ranking?.class_position ?? null,
    };

    // Dynamic import — avoids "flushSyncCallbacks" / "Uncaught undefined" error
      ...
      <CardContent className="p-4">
        ...
        <div className="flex justify-between items-center pt-3 border-t border-gray-100">
          ...
          <Button
            ...
            title="Download Detailed Resultslip"
...
// ─── SubjectAnalysisDialog — shows summative chart + both summative & formative tables ──
const SubjectAnalysisDialog = React.memo(({
  ...
  const chartData = summative
    .map((a: any) => ({
      exam:     a.assessments?.title,
      score:    a.percentage,
      date:     a.assessment_date,
      fullName: a.assessments?.title,
    }))
  ...
  return (
    <Dialog open={!!selectedSubject} onOpenChange={onClose}>
      <DialogContent
        ...
        className="max-w-[95vw] sm:max-w-4xl max-h-[85vh] overflow-y-auto bg-gradient-to-br from-white to-maroon/5 border-maroon/20 p-3 sm:p-4 md:p-6"
      >
                ...
                {summative.length > 0 && (
                  <Card className="bg-white border-maroon/20 shadow-sm">
                    <CardHeader className="bg-gradient-to-r from-maroon/5 to-transparent border-b border-maroon/10 px-3 sm:px-6 py-3">
                      ...
                      <CardDescription className="text-xs sm:text-sm text-gray-600">
                        Exams with scores and percentages
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="p-2 sm:p-6">
...
export default function Assessments({
  ...
  const fetchAssessmentsData = async () => {
    ...
    try {
      ...
      const processed = (data || []).map((item: any) => ({
        ...
        percentage:
          item.assessments?.max_marks > 0 && item.score !== null
            ? (item.score / item.assessments.max_marks) * 100
            : null,
      }));
  ...
  const handleSubjectClick = async (subjectName: string) => {
    ...
    if (!subjectId) { console.warn("No subject ID found for", subjectName); setAnalysisLoading(false); return; }

    try {
      // Fetch ALL published results for this subject (both summative and formative)
  ...
  const exams = useMemo(() => {
    ...
    return Array.from(seen)
      .sort((a, b) => {
        const na = extractExamNumber(a), nb = extractExamNumber(b);
  ...
  const pivotData = useMemo(() => {
    ...
    if (studentRankings.length > 0) {
      ...
      studentRankings.forEach((r: any) => {
        if (r.exam_title) rankingMap[r.exam_title] = r;
      });
                ...
                {Array.from(groupedByStrand.entries()).map(([strand, items]) => (
                  <Card key={strand}>
                    <CardHeader className="py-3">
                      <CardTitle className="text-sm sm:text-base">{strand}</CardTitle>
                    </CardHeader>
                          ...
                          <TableBody>
                            {(items as any[]).map((a: any) => (
                              <TableRow key={a.id}>
                                ...
                                <TableCell>
                                  {a.is_absent ? "Absent" : a.score !== null ? `${a.score}/${a.assessments?.max_marks}` : "-"}
                                </TableCell>
                                <TableCell>
                                  {a.is_absent ? (
                                    <Badge variant="outline">Absent</Badge>
                ...
                {performanceInsights && (
                  <Card className="bg-maroon/5">
                    <CardHeader className="px-4 sm:px-6">
                      <CardTitle className="text-sm sm:text-base">Performance Insights</CardTitle>
                    </CardHeader>
