...
// Raw subject shape returned by Supabase join — no is_optional (that lives on classes_subjects row)
interface RawSubjectJoin {
  id: string;
  name: string;
  code: string;
}

// Enriched shape used throughout the component after merging is_optional from the parent row
interface SubjectInfo extends RawSubjectJoin {
  is_optional: boolean;
}
...
interface TeacherClassSubjectRow {
...
interface OverviewClassEntry {
  className: string;
  gradeLevel: string;
  subjects: Record<string, OverviewSubject>;
}

interface AssignedSubjectRow {
  id: string;
  is_optional: boolean;
...
// Kept functionally identical to original per instructions — only the
// Promise.all is preserved and we guard against null data shapes.

async function fetchStudentsForClass(classId: string): Promise<StudentInClass[]> {
  ...
  (profilesRes.data ?? []).forEach((p) => { if (p.student_id) profileMap[p.student_id] = p; });
...
export default function ClassesSection() {
  ...
  const { data: classes, isLoading: loadingClasses, error: classesError } = useQuery<ClassRecord[]>({
    ...
    // FIX: staleTime prevents re-fetching every time the user switches tabs,
    // which was the primary trigger for the white screen crash on navigation.
    staleTime: 2 * 60 * 1000,
    queryFn: async () => {
      const [classesRes, csRes, enrollRes] = await Promise.all([
        ...
        supabase
          ...
          // FIX: Select only required columns instead of implicit wildcard via relation
          .select('id, class_id, subject_id, is_optional, subjects(id, name, code)'),
        supabase
          .from('enrollments')
          // FIX: Only fetch class_id for counting — avoids pulling all columns for 800 rows
          .select('class_id'),
      ]);
      ...
      (csRes.data ?? []).forEach((cs: {
        ...
        subject_id: string;
        is_optional: boolean;
        // Supabase returns joined relations as an array even for single rows;
        // RawSubjectJoin has no is_optional — that field lives on the cs row itself.
        subjects: RawSubjectJoin | RawSubjectJoin[] | null;
      }) => {
        if (!subjectsMap[cs.class_id]) subjectsMap[cs.class_id] = [];
        // Normalise array vs object shape defensively
        const subj = Array.isArray(cs.subjects) ? cs.subjects[0] : cs.subjects;
        if (subj) {
          subjectsMap[cs.class_id].push({ ...subj, is_optional: cs.is_optional });
        }
      });
  ...
  const { data: subjects, isLoading: loadingSubjects } = useQuery<SubjectRecord[]>({
    queryKey: ['subjects'],
    // FIX: Subjects rarely change — cache for 5 min to avoid refetch on tab switch
    staleTime: 5 * 60 * 1000,
    queryFn: async () => {
  ...
  const { data: gradeLevels, isLoading: loadingGradeLevels } = useQuery<GradeLevel[]>({
    ...
    // FIX: Grade levels almost never change — cache for 10 min
  ...
  // FIX: Rebuilt to query teacher_class_subjects view with only required columns.
  // select('*') removed — now explicit. staleTime added to prevent tab-switch refetch.
  const { data: overviewData, isLoading: loadingOverview } = useQuery<Record<string, OverviewClassEntry>>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          'teacher_id, teacher_name, class_id, class_name, grade_level, subject_id, subject_name, subject_code'
        );
      if (error) throw error;
      ...
      (data ?? []).forEach((row: TeacherClassSubjectRow) => {
        // FIX: Guard against null class_id rows to prevent map key corruption
        ...
        // FIX: Guard against null subject_id
        if (row.subject_id) {
  ...
  const assignSubjectsMutation = useMutation<
    void,
    Error,
    { classId: string; subjects: string[]; optionalSubjects: string[] }
  >({
      ...
      if (subjectIds.length > 0) {
        const { error: ins } = await supabase.from('classes_subjects').insert(
          subjectIds.map((sid) => ({
            class_id:    classId,
            subject_id:  sid,
            is_optional: optIds.includes(sid),
          }))
        );
        if (ins) throw ins;
      }
  ...
  const handleClassClick = (cls: ClassRecord) => {
  ...
  const handleSubjectSelection = (subjectId: string) => {
    ...
    setSelectedSubjects((prev) =>
      nowSelected ? [...prev, subjectId] : prev.filter((id) => id !== subjectId)
    );
    // FIX: Original had a bug — it checked selectedSubjects before state update.
    // Now we use nowSelected (the intended post-toggle state) to correctly
    // remove from optionals only when the subject is being deselected.
    if (!nowSelected) {
  ...
  // FIX: Added promotionClass.id to dependency check inside effect body to guard
  // against stale closure where promotionClass changes but effect fires with old data.
  React.useEffect(() => {
    if (!promotionStudents?.length || !promotionClass?.id) return;
    const currentClassId = promotionClass.id;
    setDecisions((prev) => {
      const next = { ...prev };
      ...
      // FIX: Remove any decision entries that don't belong to current promotion class
      // students — prevents stale decisions from a previous promotion session leaking in.
      const currentStudentIds = new Set(promotionStudents.map((s) => s.id));
      Object.keys(next).forEach((id) => {
        if (!currentStudentIds.has(id)) delete next[id];
      });
      void currentClassId; // consumed in closure for lint
      return next;
    });
  ...
  const setStudentDecision = (studentId: string, update: Partial<PromotionDecision>) => {
    setDecisions((prev) => ({
      ...prev,
      [studentId]: { ...prev[studentId], ...update },
    }));
  };
  ...
  const toggleSelectAll = () => {
    ...
    setSelectedForBulk(
      selectedForBulk.length === promotionStudents.length
        ? []
        : promotionStudents.map((s) => s.id)
    );
  };
  ...
  const runPromotion = async () => {
    ...
    if (invalid.length > 0) {
      setPromotionError(
        `${invalid.length} student(s) marked "Promote" have no destination class selected.`
      );
      return;
    }
    ...
    if (toRemove.length > 0) {
      await supabase
        .from('enrollments')
        .delete()
        .in('student_id', toRemove.map((d) => d.studentId));
    }

    const counts: Record<PromotionStatus, number> = {
      promoted: 0, retained: 0, graduated: 0, transferred: 0,
    };
  ...
  const promoCounts: Record<PromotionStatus, number> = {
    promoted: 0, retained: 0, graduated: 0, transferred: 0,
  };
  ...
  // FIX: Added null guards throughout — if overviewData or classes is undefined/loading,
  // returns [] instead of throwing. Protects against the race condition where classes
  // resolves before overviewData, which previously caused the memo to crash mid-render.
  const overviewClasses = React.useMemo(() => {
    ...
    return classes
      ...
      .map((cls) => {
        // FIX: If overviewData hasn't loaded yet, overviewSubjects is safely []
        // instead of throwing on undefined access.
        const overviewEntry = overviewData?.[cls.id];
        const overviewSubjects: OverviewSubject[] = overviewEntry
          ? Object.values(overviewEntry.subjects)
          : [];

        // FIX: Guard against null subject code/name before comparison
        ...
        <CardContent>
          <Tabs defaultValue="classes">
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Create and manage classes, assign subjects and view students
                  </p>
                </div>
                              ...
                              <div className="flex flex-wrap gap-1 mt-3">
                                {cls.subjects.slice(0, 3).map((s) => (
                                  <Badge
                                    ...
                                    className={`text-xs ${s.is_optional ? 'border-blue-200 text-blue-700 bg-blue-50' : ''}`}
                                  >
            ...
            <TabsContent value="overview" className="space-y-4">
              ...
              {/* FIX: Wrapped in ErrorBoundary so a single bad row never whites out the tab */}
                    ...
                    <CardContent className="p-10 text-center text-gray-400">
                      ...
                      <p className="text-sm mt-1">
                        {overviewFilter
                          ...
                          : 'Add classes and assign subjects to see the overview'}
                      </p>
                    </CardContent>
                  </Card>
                ) : (
                          ...
                          <div className="flex items-center justify-between px-5 py-3 bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-100">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-lg bg-green-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                                {/* FIX: Guard against empty/null name before charAt */}
                                {cls.name?.charAt(0) ?? '?'}
                              </div>
                              <div>
                                      ...
                                      <TableCell>
                                        <div className="flex items-center gap-2">
                                          <div className="w-6 h-6 rounded-full bg-green-100 flex items-center justify-center text-green-700 text-xs font-bold flex-shrink-0">
                                            {/* FIX: Guard teacherName before charAt */}
              ...
              {loadingGradeLevels ? (
                ...
                <Card>
                  <CardContent className="p-8 text-center">
                    ...
                    >
                      <Plus className="w-4 h-4 mr-2" />Create First Grade Level
                    </Button>
                  </CardContent>
                </Card>
              ) : (
                      ...
                      <CardContent className="p-6">
                        <div className="flex justify-between items-start">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (window.confirm(
                                `Delete grade level "${level.stage}"? This will affect classes using this grade level.`
                              )) {
          ...
          <form onSubmit={handleClassSubmit}>
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                ...
                <Input
                  id="name" name="name"
                  placeholder="e.g., Form 1A, Grade 2B"
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <BookOpen className="w-5 h-5" />Class: {selectedClass?.name}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <Card>
                ...
                <CardTitle className="text-lg flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => { setShowClassDetailModal(false); if (selectedClass) openPromotion(selectedClass); }}
                    ...
                    <Button
                      variant="outline" size="sm"
                      onClick={() => selectedClass && handleAssignSubjects(selectedClass)}
                    >
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{selectedClass?.grade_level}</p>
                  </div>
                  <div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg flex items-center gap-2">
                  <Users className="w-5 h-5" />Students in Class ({classStudents?.length ?? 0})
                </CardTitle>
              </CardHeader>
              <CardContent>
                {loadingStudents ? (
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  if (window.confirm(
                                    ...
                                    removeStudentMutation.mutate(student.enrollmentId);
                                  }
                                }}
                ...
                {assignedSubjects && assignedSubjects.length > 0 ? (
                          ...
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              {assignment.is_optional && (
                                <Badge className="bg-blue-100 text-blue-700 text-xs mt-1">Optional</Badge>
                              )}
                            </div>
                            <Button
                              variant="ghost" size="icon"
                              onClick={() => {
                                if (window.confirm(`Remove ${assignment.subjects?.name} from this class?`)) {
                                  removeSubjectMutation.mutate(assignment.id);
                                }
                              }}
                            >
                              <X className="w-4 h-4 text-red-500" />
                  ...
                  <div className="text-center py-8">
                    ...
                    <Button
                      ...
                      onClick={() => selectedClass && handleAssignSubjects(selectedClass)}
                    >
                      <Plus className="w-4 h-4 mr-2" />Assign Subjects
                    </Button>
                  </div>
                )}
              </CardContent>
        ...
        <DialogContent className="max-w-5xl max-h-[92vh] overflow-y-auto">
          ...
          {promotionDone && promotionResult ? (
            <div className="flex flex-col items-center py-10 gap-5">
              ...
              <div className="grid grid-cols-4 gap-4 w-full max-w-sm">
                {(
                  ...
                  ] as { key: PromotionStatus; label: string; color: string }[]
                ).map(({ key, label, color }) => (
              ...
              <div className="flex gap-3">
                <Button
                  variant="outline"
                  onClick={() => { setShowPromotionModal(false); setPromotionDone(false); setPromotionResult(null); }}
                >
                  Close
                </Button>
                <Button onClick={() => { setPromotionDone(false); setPromotionResult(null); }}>
                  Run Again / Correct
                </Button>
              </div>
            </div>
            ...
            <>
              ...
              {/* FIX: Wrapped promotion table in ErrorBoundary to prevent modal crash
                  from killing the entire page if a student record has unexpected shape */}
                                ...
                                <TableCell>
                                  <div className="flex items-center gap-2">
                                    <div className="h-7 w-7 rounded-full bg-blue-100 flex items-center justify-center text-xs font-bold text-blue-700 shrink-0">
                                      {/* FIX: Guard initials against undefined first/last name */}
                                ...
                                <TableCell>
                                  <div className="flex gap-1">
                                    {(
                                      ...
                                      ] as { s: PromotionStatus; emoji: string; title: string }[]
                                    ).map(({ s, emoji, title }) => (
                                      <button
                                        key={s} title={title}
                                        onClick={() => setStudentDecision(student.id, {
                                          status: s, toClassId: '', transferSchool: '',
                                        })}
                                  ...
                                  {d.status === 'promoted' && (
                                    <select
                                      className={`text-xs border rounded px-2 py-1 w-full bg-white ${
                                        !d.toClassId ? 'border-red-300' : 'border-input'
                                      }`}
                                      value={d.toClassId ?? ''}
              ...
              <DialogFooter className="pt-2">
                ...
                <Button
                  ...
                  disabled={
                    runningPromotion ||
                    loadingPromotionStudents ||
                    !promotionStudents?.length ||
                    invalidDecisions.length > 0
                  }
                >
                  {runningPromotion
                    ? 'Processing…'
                    : `Confirm & Apply (${promotionStudents?.length ?? 0} students)`}
                </Button>
              </DialogFooter>
            </>
          )}
