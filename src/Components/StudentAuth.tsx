...
export default function StudentAuth({ onLogin }: StudentAuthProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      /**
       * FLICKER FIX: 
       * Using { replace: true } prevents the "Back" button from showing the login 
       * page again and ensures an instant route transition.
       */
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            <input
              ...
              className="w-full px-4 py-3 bg-slate-800/50 border border-white/10 rounded-xl text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-600/50 focus:border-blue-600 transition-all uppercase"
              required
            />
          </div>
          <div>
          ...
          <p
            ...
            /* FIXED: Changed from window.location.href to navigate() to prevent white flash */
